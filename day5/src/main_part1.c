...
static inline bool ordering_exists(const ruleset_t *rulesets, uint32_t lpage, uint32_t rpage) {
...
int main(int argc, char* argv[])
{
    ...
    ruleset_t *rulesets = malloc(sizeof(ruleset_t) * (MAX_PAGE_NUM + 1u));
    ...
    uint64_t sum_correct_updates = 0u;
    while (!feof(in)) {
        fgets(line, MAX_LINE_LENGTH, in);
        ...
        while ((num_printed_pages < MAX_PRINT_PAGES) && (token != NULL)) {
            ...
            for (uint32_t i = 0u; i < num_printed_pages - 1u; i++) {
                if (ordering_exists(rulesets, printed_pages[num_printed_pages - 1u], printed_pages[i])) {
        ...
        if (update_is_correct) {
            sum_correct_updates += printed_pages[num_printed_pages / 2];
        }
    }
   
    printf("Sum of middle page number of correct updates = %llu", sum_correct_updates);
