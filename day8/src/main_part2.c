                                                  ...
                                                  uint32_t size_x, uint32_t size_y)
{
    ...
    for (uint32_t ay = 0u; ay < size_y; ay++) {
        for (uint32_t ax = 0u; ax < size_x; ax++) {
            ...
            int32_t offset = 0u;
            int32_t antinode_x = x - diff_x * offset;
            int32_t antinode_y = y - diff_y * offset;
            while (coordinates_valid(antinode_x, antinode_y, size_x, size_y)) {
                map[CALC_OFFSET(antinode_x, antinode_y, size_x)].is_antinode = true;
                offset++;
                antinode_x = x - diff_x * offset;
                antinode_y = y - diff_y * offset;
            }
            offset = 0u;
            antinode_x = ax + diff_x * offset;
            antinode_y = ay + diff_y * offset;
            while (coordinates_valid(antinode_x, antinode_y, size_x, size_y)) {
                map[CALC_OFFSET(antinode_x, antinode_y, size_x)].is_antinode = true;
                offset++;
                antinode_x = ax + diff_x * offset;
